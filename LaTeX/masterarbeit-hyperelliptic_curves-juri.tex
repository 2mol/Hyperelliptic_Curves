...
\begin{case} {\scshape Four Distinct Point-Components:}
  ...
  \begin{align}
    C(x)-\left(p(x)\right)^2 = -p_3^2(x-x_1)(x-x_2)(x-x_3)(x-x_4)(x-x_5)(x-x_6) %xxx
  \end{align}
  ...
  \begin{align}
  (x-x_1)(x-x_2)(x-x_3)(x-x_4)(x-x_5), %xxx
  \end{align}
  again for $x_5$ somewhere in $\bar K$. Defining $T_{4\infty}=p_2^2 - a$ and comparing the coefficients at $x^4$ gives
...
\begin{case}
  ...
  \begin{flalign*}
    && -p_3^2(x-x_1)^2&\prod_{i=3}^6(x-x_i)&\\
    \rlap{or} &&
    (x-x_1)^2&\prod_{i=3}^5(x-x_i).
  \end{flalign*}
...
\begin{case}
  ...
  \begin{flalign*}
    && -p_3^2(x-x_1)^3&\prod_{i=4}^6(x-x_i) &\\
    \rlap{or} && (x-x_1)^3&\prod_{i=4}^5(x-x_i),
  \end{flalign*}
  this again allows us to continue with one of the second steps of case 1 to find $\P_3$.
\end{case}
...
\begin{case}
  ...
  \vspace{-8mm}
  \begin{flalign*}
  \rlap{so either} && D(x) &= -p_3^2(x-x_1)^4(x-x_5)(x-x_6) &\\
  \rlap{or} && D(x) &= (x-x_1)^4(x-x_5)
  \end{flalign*}
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    %We have $\lp (f) = \gamma t^{-3} + \cdots$ and so $\lp (f - \bar f) = \delta t^{-2} + \cdots$ because the order at infinity of $f - \bar f \in K(x)$ must be even by Lemma \ref{two} (c).
...
\begin{lemma}\label{mult}
  ...
  \begin{proof} %The statement has already been proven true for $e, i, k \leq 4$ in each of the different cases.
    By construction we have% for $D(x) = C(x) - p^2(x)$
    \begin{align*}
...
\begin{theorem}
  ...
  \begin{align*}\label{f}\tag{$\star$} %xxx shitty tag
...
\newpage
  ...
  In all the other cases we have the polynomial $p(x)$ at our disposal.  We can now assume that we are not in the zero case. Since $y_i = p(x_i)$ for all $i$ except possibly $i=6$, we define
  \begin{align*}
  ...
  Let's first assume that all $\q_i \in \enkb$. This means that we are in the case where $p(x)$ is of degree 3, so as $\li (x) = t^{-2}$ we see that $\ordi q = -6$. As $q \in K[x,y]$, the only negative order may be at $\infty$ which implies $\ordq q = 0$ for all $\q \in \enkb$ that are not one of the $\q_i$. % nope. later
  ...
  \end{align*}
  completing the first part of our proof.
