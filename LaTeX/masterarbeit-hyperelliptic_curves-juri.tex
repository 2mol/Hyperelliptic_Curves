...
\vspace{-2.5mm}
  ...
  \begin{align*}\tilde V=
    ...
    \end{pmatrix}
  \end{align*}
  % Of course $\mathbf{p}=(p_0 \ p_1 \ p_2)^t$ and $\tilde \mathbf{y}=(y_1 \ y_2 \ y_3)^t$ are defined as can be expected.
  % $i=1,..,3$ which incidentally is the upper-left 3$\times$3 sub-matrix of $V$. Here $\mathbf{p}=(p_0 \ p_1 \ p_2)^t$ and $\tilde \mathbf{y}=(y_1 \ y_2 \ y_3)^t$ are defined as expected.
...
%There is a possibility for ambiguity of ``$\infty$'', which we will have to avoid later on.

%Concerning the ambiguity of ``$\infty$'' here, we will have it obvious which infinity we are working with.

%To avoid the obvious clash of notations here, we will make sure to avoid any ambiguity about 
...
\begin{lemma}\label{two}
  ...
  \begin{proof}
    ...
    \end{enumerate}
    Generally, if $f \in K(x)$ we can write $f = \frac r q$ for $r, q \in K[x]$ and apply $\ordx f = \ordx r - \ordx q$ in order to use the above on $r$ and $q$. % check this through?
  \end{proof}
\end{lemma}
...
\begin{lemma}\label{onepol}
  ...
  \begin{proof}
    ...
    \end{flalign*}
    and so on, so $\li (y - f^5 - \beta_4 f^4 - \beta_3 f^3 - \beta_2 f^2 - \beta_1 f)$ is a power series as well. Again $y = f^5 + \beta_4 f^4 + \beta_3 f^3 + \beta_2 f^2 + \beta_1 f + \beta_0 = Y(f)$ with $Y(T)\in \bar K[T] \setminus \bar K$.% because as above we can take the minimal order for every $\q \in \enkb$ of the terms in the sum $y - Y(f)$ to get $\ordq (y - Y(f)) \geq 0$.
    

    Combined we have the polynomial equation $Y(f)^2 - C(X(f)) = 0$ over $\bar K$ whose algebraic closure dictates either $f \in \bar K$ or $Y(T)^2 = C(X(T))$. The latter can't be true by Theorem \ref{satz1} and the former is in contradiction to $\ordi f = -1$.%, so actually $\ordi f \geq 0$.% so $\left. Y(T)^2 - C(X(T)) \right|_{T=f} = 0$. This means that either
  \end{proof}
\end{lemma}
...
\begin{lemma}\label{mult}
  ...
  \begin{align*} %(xxx, reminder: $D(x) = C(x) - p^2(x)$ OR $D(x) = \prod ...$)
    D(x) = (x-x_k)^e g(x)
    % (x-x_k)^e \mid D(x).
  \end{align*}
  ...
  \begin{proof} %The statement has already been proven true for $e, i, k \leq 4$ in each of the different cases.
    ...
    \end{align*}
    So by construction, $\q_i$ being equal to some $\q_k$ contributes a factor $x-x_k$.    %So by construction, $\q_5$ or $\q_6$ being equal to some other $\q_k$ contributes an additional factor $x-x_k$.
    %
...
\begin{theorem}
  ...
  \begin{align*}%\label{f}\tag{$\earth$} %xxx shitty tag
...
%   Looking back at Remark \ref{permut}
...
\begin{proof}\textit{First part}
  ...
  %Note that if $\P_2 = 0$ then we want $(f) = \q_1 + \q_2 + 2 \infty - \q_1 - \q_2 - 2 \infty = 0$, a condition that is fulfilled by $f = 1$. xxx
  %Assume that we are not in 
  %xxx (change order) In all the other cases we have the polynomial $p(x)$ at our disposal.  We can now assume that we are not in the zero case. Since $y_i = p(x_i)$ for all $i$ except possibly $i=6$, we define
  Exclude the zero case from now on, so for the remaining cases we have the polynomial $p(x)$ at our disposal. As $y_i = p(x_i)$ whenever $\q_i \neq \infty$, we define%for all $i$ except possibly $i=6$, we define
  \begin{align*}
  ...
  %XXXxxx As $q \in K[x,y]$, the only negative order may be at $\infty$ which implies $\ordq q = 0$ for all $\q \in \enkb$ that are not one of the $\q_i$. 
  ...
  \end{align*}
  As in Lemma \ref{mult}, $e_i$ denotes the exact number of occurrences of $\q_i$. Since $\sum e_i = 6$ and $q \in K[x,y]$ implies the only negative order may be at $\infty$, we have $\ordq q = 0$ for all $\q \in \enkb$ that are not one of the $\q_i$.

  %Now $\ordqi \bar q = 0$, otherwise $q - \bar q = 2y$ would imply $y_i = 0$ which we excluded.%, i.e. exactly $e_i$ of the $\q_i$ equal to some $\q_k$.

  % But by Remark \ref{alldiff} we know that $\ordqi \bar q = 0$ as $q(\bar \q_i)$ can't be zero if $\bar \q_i \neq \q_i$, so $\ordqi q = e_i$.
  ...
  %so now the first part of the proof is covered.
...
\begin{theorem}
  ...
  \begin{proof}
    ...
    \begin{alignat*}{7}
      ...
      = &&&&&&&&&&&&& \hspace{-35mm} \u + \v - \a - \b.
    \end{alignat*}
    % Of course we leave away the cases where any of the $\P \Q \R$ are zero %%nope, not necessary (?)
    ...
    This leaves $\tilde f = (x-a)(x-b)f$ which has divisor $(\tilde f) = \bar \a + \bar \b+ \u + \v - 4 \infty$. By Lemma \ref{fourpoles}, $\tilde f$ must be of the form $\kappa (x-\varrho)(x-\varsigma) \in \bar K [x]$ and so
  \begin{align*}
