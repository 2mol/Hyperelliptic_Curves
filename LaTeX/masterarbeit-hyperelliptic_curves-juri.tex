...
% As it turns out our addition law - interesting properties...
...
More broadly represented in literature is the construction of the Jacobian out of formal divisors on the function field of the curve \cite{cassels, zucc}. As a comparison, we first derive the addition law and will then prove the existence of a rational function with a divisor depending on a given sum. %This an is an approach that basically runs oppo
...
% xxxThe more standard way to define the Jacobian is more geared towards  xxxOther ways to define the Jacobian are more geared towards applications in xxx and it would be interesting to study the equivalence of both constructs .
...
% A lot of work has been done on elliptic curves, some on hyperelliptics as well. xxx insofar different xxx special attraction genus two is twofold:
...
\vspace{-2.5mm}
  ...
  % This can be seen as the $x_i$ still being pairwise distinct, only that one of them can now be $\infty$.

  % We will see later why this extension is not partial but complete.
...
\begin{figure}[ht]
  ...
  \begin{center}
    ...
    %\footnote{so $\{ \q , \bar \q \} \sim 0$ and why in $\mathds{R}^2$ it can be seen as the limit-case of $(x_3, y_3)$ approaching $(x_4, -y_4)$. The steeper segment between $\q_3$ and $\q_4$, the steeper the tangent at $\q_1$ and $\q_2$}

    % The addition with zero and illustrating $\{ \q , \bar \q \} \sim 0$ as a limit-case if $x_3$ and $x_4$.
  \end{center}
  \vspace{-1.5mm}
  \fline
\end{figure}
...
\begin{theorem}\label{hhs}
  Let $\q_1,..,\q_6$ be in $\ekb$ with %$\q_1,\q_2,\q_3,\q_4,\q_5,\q_6$
...
Since $q \in K[x,y]$, the only negative order may be at $\infty$. With $\sum_Q e_i = 6$ and $\ordi q = -6$, this implies $\ordq q = 0$ for all $\q \not \in Q$.% that don't belong to the set $\{ \q_i \}$. %are not some $\q_i$.

%\footnotetext[1]{We have to take this sum over all \emph{distinct} $\q_i$ as to avoid counting xx}
...
%   \R= \{\r_1,\r_2\},
%   \\
%   \rlap{\hspace{-1cm} and} \T = \P + \Q \text{ \hspace{4mm}with\hspace{3.5mm} } \T=\{\t_1,\t_2\},
%   \\
%   \text{\hspace{1.5mm}}\W = \Q + \R \text{ \hspace{4mm}with\hspace{1.7mm} } \W=\{\w_1,\w_2\},
%   \\
%   \S = \{\a, \b \} = \T + \R \text{, }
%   \S' = \{\u, \v \} = \P + \W \text{, }
%   \\
...
\begin{lemma}\label{xp}
  ...
  \begin{proof}
    ...
    If $x = \frac g h \in K(t)$, $g, h \in K[t]$ coprime, we have $x' = \frac{g' h - g h'}{h^2}$. Now $x' = 0$, so $g'h = gh'$. This means that $\deg g = \deg h$. If $g' = h' = 0$ then we're done.% as per the above. 
    ...
    %we simply apply the above to $g$ and $h$, so $g, h \in K[t^p]$ and thus $x \in K(t^p)$.
  \end{proof}
\end{lemma}
