...
\begin{lemma}\label{mult}
  ...
  \begin{proof}
    ...
    %and are true for $\q_5$ or $\q_6$ by construction: are equal to any $q_k$, $k\in \{1,..,6\}$ then the origin of this xxx
  \end{proof}
\end{lemma}
...
\newpage
  ...
  Let's first assume that all $\q_i \in \enkb$. This means that we are in the case where $p(x)$ is of degree 3, so as $\li : x \mapsto t^{-2}$, $\ordi q = -6$. As $q \in K[x,y]$, the only negative order may stem from $\li$ which implies $\ordq q = 0$ for all $\q \in \enkb$ that are not one of the $\q_i$.
