...
\begin{lemma}
	If $f \in \F^*$ has at most one pole at $\infty$ meaning $\ordi f \geq -1$ and none for any other $\q \in \enkb$ meaning $\ordq f \geq 0$ then $f$ is a constant.
	
	\begin{proof}
		With $f = g+hy$ such that $\ordq f \geq 0$ for every $\q \in \enkb$ we can assume that $\ordi f = -1$ and $\li (f) = \frac 1 t + \dots$. If the trailing coefficient of $\li (f)$ were different from one, we can always xxx where $\alpha \in \bar K$. Due to Lemma~\ref{nopol} we assume that $\alpha \neq 0$ and may even take $\alpha = 1$ by replacing $f$ with $\frac f \alpha$.
