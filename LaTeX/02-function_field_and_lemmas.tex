...
\begin{lemma}\label{nopol}
	...
	\begin{proof}
	  ...
	  But in $\bar K(x)$, a function $q$ with $\ordx q \geq 0$ for every $x_0 \in \bar K \cup \{ \ii \}$ must be constant, so both $f + \bar f$ and $f \bar f$ are constant functions. Since $f$ is a root of $(T-f)(T-\bar f) = T^2 - (f + \bar f)T + f \bar f$ which lies in $\bar K[T]$, $f$ lies in $\bar K \hspace{0.5mm} \cap \ \F^*$.
	\end{proof}
\end{lemma}
...
\begin{lemma}\label{onepol}
	Suppose $f \in \F^*$ has at most one pole at $\infty$ i.e. $\ordi f \geq -1$ and none for any other $\q \in \enkb$. Then $f$ is a constant $\alpha \in K$.
...
\begin{lemma}\label{twopol}
	...
	\begin{proof}
		Because $\li (f) = \beta t^{-2} + \dots$ where $\beta \in K$ we have $\li (f - \beta x)= \gamma t^{-1} + \dots$ and thanks to Lemma \ref{onepol} we know this to imply $f - \beta x = \alpha$.
	\end{proof}
\end{lemma}
...
\begin{lemma}
	...
	\begin{proof}
		...
		\begin{align*}
			\li (\frac{y}{x-\alpha_1}) &= (t^{-2} - \alpha_1)^{-1} t^{-5} \sqrt{\tau_3}\\
		...
		\end{align*}
		and $\ordq (f) \geq 1$ so $\gamma$ must be 0 since $f$ does not have any poles in $\enkb$.
...
% \textbf{Minilemma 1}: If $f$ is such that $\li (f)=\gamma t^e + \dots$ then $\li (\bar f)=-\gamma t^e + \dots$

% \begin{proof}
% 	Let $\li (\bar f) = \tilde \gamma t^e + \dots$. Since $f + \bar f = 2p$ and ord$_\infty(p)$ is always even for $p \in K(x)$, we must have $(\gamma + \tilde \gamma)t^{-3} + \dots = \sigma t^{-2} + \dots$ so $\gamma = - \tilde \gamma$.
% \end{proof}

% \textbf{Lemma 2:} Let $f \in \F^*$ with $(f)=\frac{***}{3\infty}$. Then $f = \alpha + \beta x$.
% \begin{proof}
% 	Let $f=p+qy$ with $p, q \in K(t)$. We have $\li (f + \bar f) = \sigma t^{-2} + \dots$ so $p = \frac{1}{2} (f+\bar f) = \alpha + \beta x$.

% 	Now, consider $f-\bar f = 2qy$. We get $\li (f-\bar f) = 2 \li (q) t^{-5} \sqrt \tau = 2 \gamma t^{-3}+\dots$ and since $\sqrt \tau$ is a a series of the form $1 + \dots$ we must have $\li (q) = \gamma t^2 + \dots$
...
% \begin{proof}
% 	(different function $f$): With $\li(f) = \gamma t^{-4}+\dots$ so $\li(f-\gamma x^2)$ being of the form $\sigma t^{-3}+\dots$ we fall into the case above and the claim follows.
% \end{proof}
% \vspace{-8mm}
% \fline
