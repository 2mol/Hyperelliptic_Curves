...
\begin{remark}
  ...
  % xxx defined not only on j but on J as well.
\end{remark}
...
\end{remark}
	...
	% xxx It is immediately clear now that if we have a well-defined addition, then $-\P = \bar \P$ because $\{ \q_1 , \q_2 \} + \{ \bar \q_1 , \bar \q_2 \} = \{ \q_1 , \bar \q_1 \} + \{ \q_2 , \bar \q_2 \}=0$.
	% xxx The property even gives us commutativity for free.
...
%xxx \subsection{Closing Remarks}
